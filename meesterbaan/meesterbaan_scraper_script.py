...
def get_functies():
    page = BeautifulSoup(requests.get('http://www.meesterbaan.nl/').content, features='html.parser')
    links = page.findAll('select', attrs={'id': 'ctl00_plhControl_VacaturesZoekenHomeXml1_ddlFuncties'})
    return [f.lower() for f in links[0].text.split('\n')[2:] if f != '']

DRIVER_PATH = r'Documents\geckodriver.exe'
OUTPUT_FILE = 'meester_baan.csv'
COLUMNS = ['Naam vacature', 'Naam school', 'Plaats', 'Sector', 'Denominatie', 'Dienstverband',
           'Functietitel', 'FTE', 'Opleiding', 'Salarisschaal', 'Plaatsingsdatum']
...
def driver_update_form_action(driver):
    # bug in code website: form_action postback wordt anders bij elke volgende pagina steeds langer,
    # tot dat deze bij pagina 46 niet meer update
...
def klik_en_get_urls_range(driver, begin, eind):
    ...
    for j in range(begin, eind+1):
        ...
        if elem:
            ...
            time.sleep(1)
            temp_lijst.extend(get_urls(BeautifulSoup(driver.page_source, features='html.parser')))
        else:
            break
...
def generate_url_list(driver):
    ...
    # check dat het de allerlaatste set met paginas is (en niet nog steeds de eerste pagina)
    if volgende_pagina:
...
def main():
    ...
    for url in urls:
        ...
        try:
            ...
            data_list.append(get_info(BeautifulSoup(response.content, features='html.parser')))
...
df = pd.read_csv('C:\\Users\\Datalab\\meester_baan.csv', delimiter=';' , encoding = "ISO-8859-1" ) 
...
df_amsterdam_filtered = df_amsterdam[df_amsterdam['Sector'].str.contains("Speciaal (Basis) Onderwijs|Basisonderwijs|Voortgezet onderwijs|Middelbaar beroepsonderwijs|Voortgezet speciaal onderwijs")==True]
...
df_amsterdam_filtered['Naam school'] = df_amsterdam_filtered['Naam school'].replace({"St. Nicolaaslyceum": "Scholengemeenschap Sint Nicolaas Lyceum voor Lyceum en Havo", 
...
ow_instellingenMRA['adres'] =  ow_instellingenMRA["STRAATNAAM"]  + " " + ow_instellingenMRA["HUISNUMMER-TOEVOEGING"]  
ow_instellingenMRA.rename(index=str, columns={"VESTIGINGSNUMMER": "BRIN_NUMMER", "VESTIGINGSNAAM": "NAAM_VOLLEDIG", "PLAATSNAAM": "plaats"}, inplace=True)
ow_instellingenMRA['NAAM_KORT'] = ""
ow_instellingenMRA['locatie'] = ""
ow_instellingenMRA = ow_instellingenMRA[['NAAM_VOLLEDIG', 'NAAM_KORT', 'adres', 'plaats', 'locatie', 'BRIN_NUMMER']]
...
ow_instellingen = ow_instellingen.loc[ow_instellingen['plaats'].isin(mra["Woonplaatsen"])]
...
for index_a, row_a in df_amsterdam_filtered.iterrows():
    for index_b, row_b in ow_instellingen.iterrows():
        if fuzz.partial_ratio(row_a['Naam school'].encode('utf-8'), row_b['NAAM_VOLLEDIG'].encode('utf-8')) > 85:
            matching_names_df = matching_names_df.append({'Naam school': row_a['Naam school'], 'Naam DUO': row_b['NAAM_VOLLEDIG'], 'Match': fuzz.partial_ratio(row_a['Naam school'].encode('ISO-8859-1'), row_b['NAAM_VOLLEDIG'].encode('ISO-8859-1'))}, ignore_index=True)
        if fuzz.partial_ratio(row_a['Naam school'].encode('utf-8'), row_b['NAAM_KORT'].encode('ISO-8859-1')) > 85:
            matching_names_df = matching_names_df.append({'Naam school': row_a['Naam school'], 'Naam DUO': row_b['NAAM_KORT'], 'Match': fuzz.partial_ratio(row_a['Naam school'].encode('ISO-8859-1'), row_b['NAAM_KORT'].encode('ISO-8859-1'))}, ignore_index=True)
matching_names_df
##
...
matching_names_df = matching_names_df.sort_values('Match', ascending=False).drop_duplicates(['Naam school'])
...
ow_totaal = pd.merge(df_amsterdam_filtered, matching_names_df, left_on='Naam school', right_on='Naam school', how='left')
ow_totaal = ow_totaal.reset_index()

ow_totaal_voll = pd.merge(ow_totaal, ow_instellingen, left_on='Naam DUO', right_on='NAAM_VOLLEDIG', how='left')
ow_totaal_voll = ow_totaal_voll[pd.notnull(ow_totaal_voll['NAAM_VOLLEDIG'])]

ow_totaal_kort = pd.merge(ow_totaal, ow_instellingen, left_on='Naam DUO', right_on='NAAM_KORT', how='left')
...
ow_vollkort.loc[(ow_vollkort['Naam school'] == "Wellantcollege Sloten") &  (ow_vollkort['Plaats'] == "Amsterdam"), 'adres'] = "Jan van Zutphenstraat 60"
ow_vollkort.loc[(ow_vollkort['Naam school'] == "Wellantcollege Sloten") &  (ow_vollkort['Plaats'] == "Amsterdam"), 'plaats'] = "Amsterdam"
ow_vollkort.loc[(ow_vollkort['Naam school'] == "Wellantcollege Linnaeus vmbo") &  (ow_vollkort['Plaats'] == "Amsterdam"), 'adres'] = "Jan van Zutphenstraat 60"
ow_vollkort.loc[(ow_vollkort['Naam school'] == "Wellantcollege Linnaeus vmbo") &  (ow_vollkort['Plaats'] == "Amsterdam"), 'plaats'] = "Amsterdam"
ow_vollkort.loc[(ow_vollkort['Naam school'] == "Wellantcollege Sloten") &  (ow_vollkort['Plaats'] == "Amsterdam"), 'locatie'] = "POINT(4.7953800999999885 52.3497342)"
ow_vollkort.loc[(ow_vollkort['Naam school'] == "Calandlyceum voor Gymnasium Atheneum Havo Vmbo locatie,  Lumion"), 'adres'] = "Anderlechtlaan 3"
ow_vollkort.loc[(ow_vollkort['Naam school'] == "St Jozefschool") &  (ow_vollkort['Plaats'] == "Amsterdam"), 'adres'] = "Kalfjeslaan 370"
ow_vollkort.loc[(ow_vollkort['Naam school'] == "St Jozefschool") &  (ow_vollkort['Plaats'] == "Amsterdam"), 'plaats'] = "Amsterdam"
ow_vollkort.loc[(ow_vollkort['Naam school'] == "Saenredam College") &  (ow_vollkort['Plaats'] == "Zaandijk"), 'adres'] = "Elvis Presleystraat 1"
ow_vollkort.loc[(ow_vollkort['Naam school'] == "Saenredam College") &  (ow_vollkort['Plaats'] == "Zaandijk"), 'plaats'] = "Zaandijk"




#welke scholen worden qua plaatsnaam niet goed gematcht?
ow_vollkort['inconsistent'] = np.where(((ow_vollkort['Plaats'] != ow_vollkort['plaats']) & ow_vollkort['plaats'].notnull()), 1, 0)
...
ow_vollkort.loc[(ow_vollkort[ "re1"].notnull() & (ow_vollkort[ "re2"].isnull())), 'FTE_cat'] = ow_vollkort[ "re1"]
ow_vollkort.loc[(ow_vollkort[ "re1"].notnull() & (ow_vollkort[ "re2"].notnull())), 'FTE_cat'] = (((ow_vollkort[ "re2"] - ow_vollkort[ "re1"]) / 2) + ow_vollkort[ "re1"])
